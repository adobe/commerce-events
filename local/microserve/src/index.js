...
function buildIndexHtml(options, data) {
    if (options.single) {
        ...
        data = data.slice(0, startOfHead) + `<base href="/">` + data.slice(startOfHead);
    }
    ...
    if (fs.existsSync(cssFile)) {
        data = data.slice(0, endOfHead) + cssLink + clientWebSocket + data.slice(endOfHead);
    } else {
