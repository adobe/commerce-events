...
import { EXTENSION, isDir, isFile, isTruthy, stdout, removeScope } from "./utils";
...
type MiniBundleOptions = {
    entry: string | string[];
    // output
    // format
    // watch -- leave this as a cli flag
    // pkg-main
    // target: "node" | "web"
    // external:
    // globals:
...
// export default async function minibundle(cliOptions: any) {
//     // const options = { ...cliOptions };
//     const explorer = cosmiconfig("minibundle");
//     const config = await explorer.search();

//     const options = normalizeOptions(cliOptions, config);
...
type MiniBundleOutput = Promise<{ output?: string; watchers?: Record<string, RollupWatcher> }>;

export default async function minibundle(cliOptions): MiniBundleOutput {
    const options = { ...cliOptions };

    options.cwd = resolve(process.cwd(), cliOptions.cwd);
    ...
    const steps: InputOutputOptions[] = [];
    for (let i = 0; i < options.entries.length; i++) {
        for (let j = 0; j < formats.length; j++) {
            steps.push(createConfig(options, options.entries[i], formats[j], j === 0));
        }
...
const normalizeOptions = async (cli, config) => {
    const options = { ...cli };

    return options;
};

function doWatch(options, cwd, steps: InputOutputOptions[]): MiniBundleOutput {
...
function createConfig(options, entry, format, writeMeta): InputOutputOptions {
    const { pkg } = options;
