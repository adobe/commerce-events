...
function getPadLeft(str: string, width: number, char = " "): string {
    return char.repeat(width - str.length);
}

function formatSize(size: number, filename: string, type: string, raw: boolean): string {
...
export async function getSizeInfo(code: string, filename: string, raw: boolean): Promise<string> {
    raw = raw || code.length < 5000;

    const [gzip, brotli] = await Promise.all([gzipSize(code).catch(() => null), brotliSize(code).catch(() => null)]);

    let out;
    if (gzip) out = formatSize(gzip, filename, "gz", raw);
    if (brotli) {
